"""
Deep Q-learning Neural Network (DQN) implementation based on Open AI gym - Atari game : Assault
...
if __name__ == '__main__':
    ...
    n_episodes = 400
    seed = 0
    accu_rewards_plot = np.array([])            # stack in accu_rewards for plot
    ...
    for i_episode in range(n_episodes):
        ...
        accu_rewards_plot = np.hstack(accu_rewards_plot, accu_rewards)   # stack for n_episodes of accu_rewards
    
    env.close() # end of simulation

    # Plot result
    t_plot = np.linspace(0, n_episodes, n_episodes)
    plt.plot(t_plot, accu_rewards_plot)
    plt.show()
